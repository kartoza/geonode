...
class OgcWmsHarvester(base.BaseHarvesterWorker):
    ...
    def __init__(self, *args, dataset_title_filter: typing.Optional[str] = None, **kwargs):
    ...
    def from_django_record(cls, record: models.Harvester):
        return cls(
            record.remote_url,
            record.id,
            dataset_title_filter=record.harvester_type_specific_configuration.get("dataset_title_filter"),
        )
    ...
    def get_extra_config_schema(cls) -> typing.Optional[typing.Dict]:
        return {
            ...
            "description": (
                "A jsonschema for validating configuration option for GeoNode's " "remote OGC WMS harvester"
            ),
            "type": "object",
            "properties": {
        ...
        try:
            ...
            for _op in _parsed_service.operations:
                ...
                for _op_method in getattr(_op, "methods", []) if hasattr(_op, "methods") else _op.get("methods", []):
                    _methods.append({"type": _op_method.get("type", None), "url": _op_method.get("url", None)})

                _name = getattr(_op, "name", None) if hasattr(_op, "name") else _op.get("name", None)
                _formatOptions = (
                    getattr(_op, "formatOptions", []) if hasattr(_op, "formatOptions") else _op.get("formatOptions", [])
                )
                if _name:
                    operations[_name] = {"name": _name, "methods": _methods, "formatOptions": _formatOptions}
    ...
    def _get_data(self) -> typing.Dict:
        ...
        for layer_element in leaf_layers:
            try:
                ...
                logger.exception(e)
        return {"contact": self._get_contact(root), "layers": layers}

    def _get_contact(self, element: etree.Element) -> dict:
        """Return contact from element"""
        nsmap = _get_nsmap(element.nsmap)
        return {
            "role": "",
            ...
            "position": get_xpath_value(element, "wms:Service/wms:ContactInformation/wms:ContactPosition", nsmap),
    ...
    def _layer_element_to_json(self, layer_element: etree.Element) -> dict:
        ...
        try:
            legend_url = layer_element.xpath("wms:Style/wms:LegendURL/wms:OnlineResource", namespaces=nsmap)[0].attrib[
                f"{{{nsmap['xlink']}}}href"
            ]
        ...
        wms_url = self.get_ogc_wms_url(wms_url._replace(query=urlencode(params)), version=_version)
        ...
        try:
            ...
            if not spatial_extent:
                crs = "EPSG:4326"
                spatial_extent = geos.Polygon.from_bbox((-180.0, -90.0, 180.0, 90.0))
        return {
            "name": name,
            "title": title,
            "abstract": abstract,
